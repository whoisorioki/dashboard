...
async def monthly_sales_growth(
    start_date: str = Query("2024-06-01", description="Start date in YYYY-MM-DD format"),
    ...
    item_names: Optional[str] = Query(None, description="Comma-separated list of item names to filter"),
    sales_persons: Optional[str] = Query(None, description="Comma-separated list of sales personnel to filter"),
    branch_names: Optional[str] = Query(None, description="Comma-separated list of branch names to filter (legacy)"),
    branch: Optional[str] = Query(None, description="Single branch name to filter"),
    product_line: Optional[str] = Query(None, description="Product line to filter (e.g., 'Piaggio', 'TVS')"),
):
    """
    ...
    **Response Format:**
    ...
    try:
        ...
        return {"using_mock_data": mock_data, "result": result}
    except Exception as e:
        if not mock_data:
            logger.warning(f"Druid failed for monthly_sales_growth, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
            ...
            if product_line and product_line != "all":
                df = df.filter(pl.col("ProductLine") == product_line)
            result = await run_in_threadpool(kpi_service.calculate_monthly_sales_growth, df)
            return {"using_mock_data": True, "result": result}
        raise
...
async def sales_target_attainment(
    ...
    try:
        ...
        if product_line and product_line != "all":
            df = df.filter(pl.col("ProductLine") == product_line)
        result = await run_in_threadpool(kpi_service.calculate_sales_target_attainment, df, target)
        return {"using_mock_data": mock_data, "result": result}
    except Exception as e:
        if not mock_data:
            logger.warning(f"Druid failed for sales_target_attainment, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
...
async def product_performance(
    ...
    try:
        ...
        if not mock_data:
            logger.warning(f"Druid failed for product_performance, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
...
async def branch_product_heatmap(
    ...
    try:
        ...
        if product_line and product_line != "all":
            df = df.filter(pl.col("ProductLine") == product_line)
        result_df = await run_in_threadpool(kpi_service.create_branch_product_heatmap_data, df)
        return {"using_mock_data": mock_data, "result": result_df.to_dicts()}
    except Exception as e:
        if not mock_data:
            logger.warning(f"Druid failed for branch_product_heatmap, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
            ...
            if product_line and product_line != "all":
                df = df.filter(pl.col("ProductLine") == product_line)
            result_df = await run_in_threadpool(kpi_service.create_branch_product_heatmap_data, df)
            return {"using_mock_data": True, "result": result_df.to_dicts()}
        raise
...
async def branch_performance(
    ...
    try:
        ...
        if not mock_data:
            logger.warning(f"Druid failed for branch_performance, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
...
async def branch_list(
    ...
    try:
        ...
        if not mock_data:
            logger.warning(f"Druid failed for branch_list, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
...
async def branch_growth(
    ...
    try:
        ...
        if not mock_data:
            logger.warning(f"Druid failed for branch_growth, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
...
async def sales_performance(
    ...
    try:
        ...
        if not mock_data:
            logger.warning(f"Druid failed for sales_performance, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
...
async def product_analytics(
    ...
    try:
        ...
        if not mock_data:
            logger.warning(f"Druid failed for product_analytics, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
...
async def revenue_summary(
    ...
    try:
        ...
        if not mock_data:
            logger.warning(f"Druid failed for revenue_summary, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
...
mock_customer_value_data = [
    {"AcctName": "Acme Corp", "total_gross_revenue": 120000.0, "total_net_profit": 35000.0},
    {"AcctName": "Beta Industries", "total_gross_revenue": 95000.0, "total_net_profit": 22000.0},
    {"AcctName": "Gamma LLC", "total_gross_revenue": 87000.0, "total_net_profit": 18000.0},
    {"AcctName": "Delta Partners", "total_gross_revenue": 65000.0, "total_net_profit": 12000.0},
    {"AcctName": "Epsilon Inc", "total_gross_revenue": 43000.0, "total_net_profit": 9000.0},
]
...
async def customer_value(
    ...
    try:
        ...
        if product_line and product_line != "all":
            df = df.filter(pl.col("ProductLine") == product_line)
        if df.is_empty():
            return {"using_mock_data": mock_data, "result": []}
        result_df = (
            df.lazy()
            .group_by("AcctName")
            .agg([
                pl.sum("grossRevenue").alias("total_gross_revenue"),
                (pl.sum("grossRevenue") - pl.sum("totalCost")).alias("total_net_profit"),
            ])
        ...
        if not mock_data:
            logger.warning(f"Druid failed for customer_value, falling back to mock data: {e}")
            df = await fetch_sales_data(
                start_date=start_date,
                end_date=end_date,
                item_names=item_names_list,
            ...
            if product_line and product_line != "all":
                df = df.filter(pl.col("ProductLine") == product_line)
            if df.is_empty():
                return {"using_mock_data": True, "result": []}
            result_df = (
                df.lazy()
                .group_by("AcctName")
                .agg([
...
async def employee_performance(
    ...
    try:
        ...
        result = await run_in_threadpool(kpi_service.calculate_employee_performance, df, quotas_df)
    except Exception as e:
        import logging
        logging.warning(f"Druid failed for employee_performance, falling back to mock data: {e}")
        using_mock_data = True
        df = await run_in_threadpool(sales_data.get_mock_sales_data, start_date, end_date)
        quotas_df = await run_in_threadpool(sales_data.get_mock_employee_quotas)
        result = await run_in_threadpool(kpi_service.calculate_employee_performance, df, quotas_df)
    return {"using_mock_data": using_mock_data, "result": result}
...
async def margin_trends(
    ...
    result = [
        {"date": (base.replace(month=((i % 12) + 1))).strftime("%Y-%m"), "margin": 20 + i % 5}
        for i in range(6)
    ]
    return {"using_mock_data": True, "result": result}
