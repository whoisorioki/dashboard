...
from services.kpi_service import calculate_revenue_summary, _ensure_time_is_datetime
import asyncio
import logging
import math
import polars as pl
...
class TargetAttainment:
    attainment_percentage: Optional[float] = strawberry.field(name="attainmentPercentage", default=0.0)
    total_sales: Optional[float] = strawberry.field(name="totalSales", default=0.0)
...
class Query:
    ...
    async def monthly_sales_growth(
        ...
        if self is None:
            ...
            return await query_instance.monthly_sales_growth(
                start_date, end_date, branch, product_line
            )
        from services.kpi_service import calculate_monthly_sales_growth
    ...
    async def product_performance(
        ...
        # Ensure n has a default value
        limit = n if n is not None else 10
    ...
    async def branch_growth(
        ...
        from services.kpi_service import calculate_branch_growth
