/**
 * Fetches the available data range and total record count from the backend /data-range endpoint.
...
export async function fetchAndLogDataRange() {
    try {
        ...
        if (result && result.data && result.data.length > 0) {
            ...
            // eslint-disable-next-line no-console
            console.warn('[Data Range] No data returned from /data-range');
        }
    } catch (error) {
        // eslint-disable-next-line no-console
        console.error('[Data Range] Error fetching /data-range:', error);
    }
} 